  ...
  test.describe("when a referenced snippet is deleted", () => {
    test.beforeEach(async ({ context, page, expectedConsoleErrors }) => {
      expectedConsoleErrors.push(
        /\[resolveSnippets\] Snippet not found: @greet/,
      );
    ...
    test("blocks regeneration and shows an error", async () => {
      ...
      // We click the button directly instead of using the POM, because this test
      // asserts that NO request is made. The POM method might change, but this test
      // should always just test the raw click and its consequences.
      const messageLocator = chatPage.getMessageLocator(1);
