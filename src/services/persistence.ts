...
export async function deleteSnippet(snippetName: string): Promise<void> {
  const db = await dbPromise;
  try {
    await db.delete(SNIPPETS_STORE_NAME, snippetName);
  } catch (error) {
    console.error(`[DB] Delete: Failed to delete snippet '${snippetName}':`, error);
